<?php
/**
 * Выводим пункты меню автодобавляя для альтернативного языка его алиас в начале, если мы находимся на этом языке. 
 * Не всегда это все надо, разумнее пользоваться ограничениями вывода пунктов меню по altlang.
 *
 ...
 * Time: 00:00
 */
class WidgetGormenulangController extends AbstractWidgetControllerController {
    ...
    /**
     * @var type текущий файл шаблона вывода виджета
     */
    public $tpl = '';
    ...
    public function action_index($item=array(),$param=NULL,$vidget_tpl=''){
        ...
        foreach ($storageRes as $menuItem) if (empty($menuItem["type"])) {
            ...
            $menuResItem['url'] = (!empty($menuItem['vars']['url']))?(((!Lang::isDefLang())?('/'.Lang::getAltLangName()):'').$menuItem['vars']['url']):'';
