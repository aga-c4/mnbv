...
работы с конфигурацией хранилиш, при этом автоматически формируется интерфейс
...
работать с консолью, используюя параметры из вызова скрипта по аналогии с GET
...
поддержка ведется паралельно для 2х языков. Весь вывод в шаблоны и т.п. ведется
...
8. Откройте ваш домен в браузере по урл http://ВАШ-ДОМЕН, проверьте 
работоспособность.

9. Для перехода в панель администрирования системы используйте URL:
http://ВАШ-ДОМЕН/intranet
login: admin
passwd: admin
