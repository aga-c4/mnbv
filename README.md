Внимание! Коллеги, в настоящий момент проводится массовый апдейт системы, оно 
в основном затронет шаблон и реализацию функций интернет-магазина, которые ранее
были в зачаточном состоянии. Основные работы планируется завершить до конца мая,
все работы до конца июня. Задача - создание полноценного интернет-магазина, 
который можно было бы использовать в серьезных проектах. 

Что интересного ожидаем:
- Гибкая система работы с атрибутами товаров.
- Автоматическая генерация каталога с фильтрами (построение иерархического дерева
фильтров) из YML файла, поддержка обмена наличием товаров с 1с и другими
системам.
- Адаптация уже реализованного модуля генерации товарных фидов.
- Расширенная система аналитики, позволяющая с помощью стандартных механизмов GA
и иных счетчиков оценивать влияние баннеров, рекомендаций на продажи конкретных
товаров.
- Время генерации страницы на ноутбуке пока около 0,05 секунд, но в конце процесса
мы загрузим около 100к товаров и проведем полноценное нагрузочное тестирование.
Кеширование минимально используется, намеренно, чтоб интегрировать его более
полно на конкретных решениях.

Не подумал, что Хайлоуд2021 подогреет какой-то интерес, однако подогрел, иначе 
изменения зарелизили бы до него)))

################################################################################
##  Фреймворк и CMS.MNBV-8 ( демо http://aga-c4.ru - скоро будет НОВАЯ ВЕРСИЯ).
################################################################################
