...
<div class="w-100">
    <script>
        function checkMainSearchForm() {
            ...
            if (document.getElementById('mnsearch').value == '') {
                 document.getElementById('mnsearch').focus();
                 return false;
            }
            document.mainSearchForm.submit();
        }
    </script>
    <form id="mainSearchForm" action="<?=MNBVf::requestUrl(Lang::isAltLang()?'altlang':'','/search');?>" class="d-flex" method="GET" onsubmit="return checkMainSearchForm();">
        <input type="search" class="form-control mr-2" name="search" id="mnsearch" placeholder="<?=Lang::get('Search string');?>" value="<?=SysBF::getFrArr($item,'search_str','');?>">
        <button class="btn btn-outline-success btn-dark text-light" onclick="checkMainSearchForm();">Search</button>
    </form>
</div>
...
<div class="col use-prods d-none my-2">
            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-6 col-lg-4 mt-2">
                    ...
                    <div>
                        ...
                        ?>     
                        <div class="checkbox"><label><input type="checkbox"<?=(!empty($attrItemVal["selected"]))?' checked':'';?> name="f<?=$attrAlias;?>[]" class="cb-<?=$attrAlias;?> flt-chkbx" value="<?=$attrItemAlias;?>"> <?=MNBVf::getItemName($attrItemVal,Lang::isAltLang());?></label></div>
                        <? 
                            } 
                        ...
                        ?>
                        <div class="checkbox <?=$attrAlias;?>-hide d-none"><label><input type="checkbox"<?=(!empty($attrItemVal["selected"]))?' checked':'';?> name="f<?=$attrAlias;?>[]" class="cb-<?=$attrAlias;?> flt-chkbx" value="<?=$attrItemAlias;?>"> <?=MNBVf::getItemName($attrItemVal,Lang::isAltLang());?></label></div>
                        <?
                                } else {
                        ?>      
                        <div class="checkbox"><label><input type="checkbox"<?=(!empty($attrItemVal["selected"]))?' checked':'';?> name="f<?=$attrAlias;?>[]" class="cb-<?=$attrAlias;?> flt-chkbx" value="<?=$attrItemAlias;?>"> <?=MNBVf::getItemName($attrItemVal,Lang::isAltLang());?></label></div>
