...
$menuStr = '<ul class="navbar-nav mr-auto">'."\n";
...
foreach ($menuArr as $menuItem) {
    $menuStr .= '<li class="nav-item">'
            . '<a ' . ((!empty($menuItem['active']))?'class="nav-link active my-nav-link"':'class="nav-link my-nav-link"') . ' href="' . $menuItem['url'] . '">' . $menuItem['name'] . '</a></li>'."\n";
}
$menuStr .= '</ul>'."\n";
echo $menuStr;
